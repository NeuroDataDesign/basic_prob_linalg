...
+ these will be vital for our understanding of analytical understanding of MLE in later steps
    + analytical: we have descriptions of characteristics of the data itself $a-priori$ (know them ahead of time)
    + empirical: we have observations of data, and need to estimate characteristics of the data
...
for (i in 1:length(ns)) {
  for (j in 1:length(ps)) {
    ...
    res <-  mean(abs(ps[j] - replicate(nsim, sum(sim.bern(ps[j],  n=ns[i])/ns[i]))))
    results[i + (j-1)*length(ps),] <- data.frame(n = ns[i], er=res, p=ps[j])
  }
}
...
ggplot(data=results, aes(x=n, y=er, group=p, color=p)) +
  ...
  ylab(TeX('mean $\\left|p_{true} - \\hat{p}\\right|$ over 100 simulations'))
```
